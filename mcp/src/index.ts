...
server.tool(
  "threadron_checkin",
  "Session start check-in. Returns your active sprints, in-progress work, pending items, blocked items, and unprocessed inbox items. Use this at the start of every session to understand what needs attention.",
  {},
  async () => {
    const [sprints, inProgress, pending, blocked, inbox] = await Promise.all([
      api(`/sprints?status=active`).then((d) => d.sprints || d).catch(() => []),
      ...
      api(`/inbox?status=unprocessed`).then((d) => d.items || d).catch(() => []),
    ]);
    ...
    const summary = {
      ...
      pending,
      blocked,
      unprocessed_inbox: inbox,
      summary: `${sprints.length} active sprints, ${inProgress.length} in progress, ${pending.length} pending, ${blocked.length} blocked, ${inbox.length} inbox items to parse`,
    };
...
server.tool(
  ...
  "Parse an unprocessed inbox item — interpret the raw text and propose structured task fields. Always include a concrete next_action. Set confidence based on how sure you are of the interpretation.",
  {
...
server.tool(
  ...
  async ({ name, domain_id, goal, start_date, end_date, status, capacity_notes }) => {
    const payload: Record<string, unknown> = { name, created_by: AGENT_ID };
    if (domain_id) payload.domain_id = domain_id;
...
server.tool(
  ...
  async ({ sprint_id, task_id, thread_id, commitment_status, position }) => {
    const payload: Record<string, unknown> = { added_by: AGENT_ID };
